...
const Testimonials: React.FC = () => {
  ...
  const animateChange = (callback: () => void) => {
    ...
    tl.to(contentRef.current, { opacity: 0, x: -20, duration: 0.3, ease: "power2.in" })
      .call(callback)
      .fromTo(contentRef.current, { opacity: 0, x: 20 }, { opacity: 1, x: 0, duration: 0.3, ease: "power2.out" });
  };
    ...
    <section className="py-24 bg-zinc-100 dark:bg-zinc-900 relative transition-colors duration-500">
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="text-center mb-16">
            <h2 className="text-yc-yellow font-rubik tracking-widest uppercase text-sm mb-2">Client Stories</h2>
            <h3 className="text-4xl font-syne font-bold text-zinc-900 dark:text-white transition-colors duration-500">RIDE EXPERIENCE</h3>
        </div>
        ...
        <div className="max-w-4xl mx-auto bg-white/50 dark:bg-zinc-950/50 border border-zinc-200 dark:border-zinc-800 p-8 md:p-12 relative backdrop-blur-sm transition-colors duration-500 shadow-sm dark:shadow-none">
            <Quote size={48} className="text-zinc-200 dark:text-zinc-800 absolute top-8 left-8 transform -scale-x-100 transition-colors duration-500" />
