...
const Hero: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      const flicker = (target: any) => {
        const ftl = gsap.timeline();
        ftl.to(target, { opacity: 1, duration: 0.05 })
  ...
  return (
    <section ref={containerRef} className="relative h-screen w-full bg-zinc-100 dark:bg-[#09090B] overflow-hidden flex flex-col justify-center items-center transition-colors duration-500">
            ...
            <div className="w-[80vw] max-w-4xl h-[60vh] bg-zinc-100 dark:bg-[#18181B] relative border-x border-t border-zinc-300 dark:border-zinc-800 shadow-2xl transition-colors duration-500">
                 ...
                 <div className="absolute inset-0 flex flex-col">
                     {Array.from({ length: 20 }).map((_, i) => (
                         <div key={i} className="flex-1 border-b border-zinc-200 dark:border-[#09090B]/80 bg-gradient-to-b from-white/20 dark:from-white/5 to-transparent"></div>
                     ))}
                 </div>
                 {/* Shutter Bottom Bar */}
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
            <div className="w-[90vw] max-w-5xl h-[70vh] flex justify-between items-center px-4 md:px-12">
                {/* Left Light */}
                <div ref={leftLightRef} className="w-2 md:w-3 h-full bg-zinc-400 dark:bg-zinc-50/80 rounded-full shadow-[0_0_50px_rgba(0,0,0,0.1)] dark:shadow-[0_0_50px_rgba(255,255,255,0.2)] opacity-10 relative">
                     <div className="absolute inset-0 bg-white blur-[2px]"></div>
                </div>
                {/* Right Light */}
                <div ref={rightLightRef} className="w-2 md:w-3 h-full bg-zinc-400 dark:bg-zinc-50/80 rounded-full shadow-[0_0_50px_rgba(0,0,0,0.1)] dark:shadow-[0_0_50px_rgba(255,255,255,0.2)] opacity-10 relative">
                    <div className="absolute inset-0 bg-white blur-[2px]"></div>
                </div>
            </div>
        </div>
