...
const Portfolio: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const ctx = window.gsap.context(() => {
        window.gsap.fromTo(".portfolio-item", 
    ...
    >
      <div className="container mx-auto px-6">
        <div className="flex flex-col md:flex-row justify-between items-end mb-16">
            <div>
                <h1 className="text-5xl md:text-7xl font-syne font-bold text-zinc-900 dark:text-white mb-6 transition-colors duration-500">WORKSHOP <span className="text-yc-yellow">ARCHIVES</span></h1>
                <p className="text-zinc-600 dark:text-zinc-400 font-rubik max-w-lg text-lg transition-colors duration-500">
                    A curated collection of our finest builds. From faithful restorations to radical custom fabrications.
                </p>
            </div>
            
            <div className="flex gap-4 mt-8 md:mt-0">
                {['All', 'Restoration', 'Modification'].map((f) => (
                    <button
                        key={f}
                    ...
                    <Link to={`/project/${project.id}`} className="block">
                        <div className="relative overflow-hidden bg-zinc-200 dark:bg-zinc-800 aspect-[4/3] mb-5 border border-zinc-200 dark:border-zinc-800 group-hover:border-yc-yellow/50 transition-all duration-500 rounded-sm">
                            ...
                            <img 
                                src={project.image} 
                                alt={project.title} 
                                className="w-full h-full object-cover transform group-hover:scale-105 transition-transform duration-700 ease-out grayscale-[0.3] group-hover:grayscale-0"
                            />
                        ...
                        <div className="flex flex-col gap-1">
                            <div className="flex justify-between items-baseline">
                                ...
                                <span className="text-zinc-500 dark:text-zinc-600 font-rubik text-xs uppercase tracking-widest">{project.year}</span>
                            </div>
