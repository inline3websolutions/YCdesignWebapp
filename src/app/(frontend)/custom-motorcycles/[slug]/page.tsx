...
export default async function CustomMotoPage({ params: paramsPromise }: Args) {
  const { isEnabled: draft } = await draftMode()
  const { slug = '' } = await paramsPromise
  const decodedSlug = decodeURIComponent(slug)
  const url = '/custom-motorcycles/' + decodedSlug
  const moto = await queryCustomMotoBySlug({ slug: decodedSlug })
              ...
              {moto.relatedMotorcycles.map((related) => {
                if (typeof related === 'object') {
                  return (
                    <a
                      key={related.id}
                      href={`/custom-motorcycles/${related.slug}`}
...
const queryCustomMotoBySlug = cache(async ({ slug }: { slug: string }) => {
  const { isEnabled: draft } = await draftMode()
