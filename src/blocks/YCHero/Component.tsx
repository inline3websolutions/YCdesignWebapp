...
type Props = Extract<Page['layout'][number], { blockType: 'ycHero' }>

export const YCHeroBlock: React.FC<Props> = (props) => {
  const { headline, location, tagline } = props
  ...
  return (
    <section
      ref={containerRef}
      className="relative h-screen w-full bg-zinc-100 dark:bg-[#09090B] overflow-hidden flex flex-col justify-center items-center transition-colors duration-500 -mt-16"
    >
      {/* --- BACKGROUND TEXTURES --- */}
      ...
      <div className="relative z-20 text-center flex flex-col items-center justify-center h-full pt-20">
        ...
        <div ref={textRef} className="opacity-0 space-y-8">
          <h1 className="text-4xl md:text-6xl font-syne font-bold text-zinc-900 dark:text-white tracking-wide uppercase drop-shadow-sm dark:drop-shadow-2xl transition-colors duration-500">
            {headline || 'Preserving Legends'}
            <span className="text-yc-yellow">.</span>
          </h1>
